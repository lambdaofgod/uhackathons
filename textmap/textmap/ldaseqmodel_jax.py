...
def _jax_compute_post_variance_scan_uncompiled(
    ...
    fwd_variance_out = jnp.zeros(
        num_time_slices + 1, dtype=dtype
    )  # Use num_time_slices directly as it's static
    ...
    initial_bwd_var_carry = fwd_variance_out[
        num_time_slices
    ]  # Use num_time_slices directly
    ...
    variance_out = jnp.zeros(
        num_time_slices + 1, dtype=dtype
    )  # Use num_time_slices directly
    variance_out = variance_out.at[num_time_slices].set(
        fwd_variance_out[num_time_slices]
    )  # Use num_time_slices
    variance_out = variance_out.at[:num_time_slices].set(
        jnp.flip(var_values_T_minus_1_to_0_rev)
    )  # Use num_time_slices
...
# JIT compile the function
jax_compute_post_variance_scan = jax.jit(
    _jax_compute_post_variance_scan_uncompiled, static_argnums=(2,)
)
...
class LdaSeqModelJax(utils.SaveLoad):
    ...
    def make_lda_seq_slice_jax(
        ...
        logger.debug(
            f"LDA model has topics attr: {hasattr(lda_model_instance, 'topics')}"
        )
        logger.debug(
            f"LDA model has expElogbeta attr: {hasattr(lda_model_instance, 'expElogbeta')}"
        )
        ...
        lda_model_instance.expElogbeta = (
            current_exp_elogbeta.T
        )  # Transpose to match LdaModel's expected shape
        logger.debug(
        ...
        logger.debug(
            f"After updates, LDA model has topics attr: {hasattr(lda_model_instance, 'topics')}"
        )
