...
from scipy.special import (
    digamma,
    gammaln,
)  # Not used in these specific functions but good for context

# Note: Constants like INIT_VARIANCE_CONST, OBS_NORM_CUTOFF, STEP_SIZE, TOL
# would typically be defined globally or passed if they need to be configurable.
# For now, I'm using values from the original code where they appear.
INIT_VARIANCE_CONST_NB = 1000.0  # Numba compatible constant name
INIT_MULT_NB = 1000.0  # Numba compatible constant name


@njit
def numba_compute_post_mean(
    obs_word: np.ndarray,  # 1D array (num_time_slices) for current word
    fwd_variance_word: np.ndarray,  # 1D array (num_time_slices + 1) for current word
    mean_word_out: np.ndarray,  # 1D array (num_time_slices + 1) to store output mean
    fwd_mean_word_out: np.ndarray,  # 1D array (num_time_slices + 1) to store output fwd_mean
    chain_variance: float,
    obs_variance: float,
    num_time_slices: int,
) -> None:
    """
    Numba-jitted version of sslm.compute_post_mean.
    Calculates posterior mean and forward mean for a single word.
    Outputs are written to `mean_word_out` and `fwd_mean_word_out`.
    """
    T = num_time_slices

    # Forward pass for fwd_mean
    fwd_mean_word_out[0] = 0.0
    for t_idx in range(1, T + 1):  # t_idx from 1 to T (corresponds to mean_1 to mean_T)
        denominator = fwd_variance_word[t_idx - 1] + chain_variance + obs_variance
        c_factor = 0.0
        if denominator != 0.0:
            c_factor = obs_variance / denominator

        fwd_mean_word_out[t_idx] = (
            c_factor * fwd_mean_word_out[t_idx - 1]
            + (1.0 - c_factor) * obs_word[t_idx - 1]
        )

    # Backward pass for smooth_mean
    mean_word_out[T] = fwd_mean_word_out[T]
    for t_idx in range(T - 1, -1, -1):  # t_idx from T-1 down to 0
        fwd_var_t_plus_chain_var = fwd_variance_word[t_idx] + chain_variance
        c_factor_backward = 0.0
        if chain_variance != 0.0 and fwd_var_t_plus_chain_var != 0.0:
            c_factor_backward = chain_variance / fwd_var_t_plus_chain_var

        mean_word_out[t_idx] = (
            c_factor_backward * fwd_mean_word_out[t_idx]
            + (1.0 - c_factor_backward) * mean_word_out[t_idx + 1]
        )


@njit
def numba_compute_mean_deriv(
    fwd_variance_word: np.ndarray,  # 1D array (num_time_slices + 1) for current word
    deriv_output: np.ndarray,  # 1D array (num_time_slices + 1) for output, d mean_s / d obs_t
    chain_variance: float,
    obs_variance: float,
    num_time_slices: int,
    time_obs_idx: int,  # Index 't' of obs_t for which derivative is taken
) -> None:
    """
    Numba-jitted version of sslm.compute_mean_deriv.
    Computes d E[beta_{s,w}] / d obs_{t,w} for s = 0..T.
    Output is written to `deriv_output`.
    """
    T_plus_1 = num_time_slices + 1

    # Initialize deriv_output, as it might be reused
    deriv_output[:] = 0.0

    # Forward pass for d(fwd_mean_s)/d(obs_t)
    for s_mean_idx in range(1, T_plus_1):  # s_mean_idx from 1 to T
        denominator = fwd_variance_word[s_mean_idx - 1] + chain_variance + obs_variance
        w_factor = 0.0
        if obs_variance > 0.0 and denominator != 0.0:
            w_factor = obs_variance / denominator

        val = w_factor * deriv_output[s_mean_idx - 1]
        if time_obs_idx == (s_mean_idx - 1):
            val += 1.0 - w_factor
        deriv_output[s_mean_idx] = val

    # Backward pass for d(smooth_mean_s)/d(obs_t)
    for s_mean_idx in range(T_plus_1 - 2, -1, -1):  # s_mean_idx from T-1 down to 0
        fwd_var_s_plus_chain_var = fwd_variance_word[s_mean_idx] + chain_variance
        w_factor_backward = 0.0
        if chain_variance != 0.0 and fwd_var_s_plus_chain_var != 0.0:
            w_factor_backward = chain_variance / fwd_var_s_plus_chain_var

        deriv_output[s_mean_idx] = (
            w_factor_backward * deriv_output[s_mean_idx]
            + (1.0 - w_factor_backward) * deriv_output[s_mean_idx + 1]
        )


@njit
def numba_compute_obs_deriv(
    current_mean_word: np.ndarray,  # 1D (T+1), mean computed with current obs_w
    current_variance_word: np.ndarray,  # 1D (T+1), fixed variance for this word
    zeta_topic: np.ndarray,  # 1D (T), fixed zeta for the topic
    word_counts_w: np.ndarray,  # 1D (T), counts for current word
    totals_time: np.ndarray,  # 1D (T), total counts over vocab per time
    mean_deriv_mtx_w: np.ndarray,  # 2D (T, T+1), precomputed d(mean_s)/d(obs_t) for current word
    deriv_output: np.ndarray,  # 1D (T), output buffer for gradient d(obj)/d(obs_t)
    chain_variance: float,
    num_time_slices: int,
) -> None:
    """
    Numba-jitted version of sslm.compute_obs_deriv.
    Computes gradient d(objective_func) / d(obs_{t,w}) for t = 0..T-1.
    Output written to `deriv_output`.
    """
    T = num_time_slices

    # Precompute temp_vect: exp(mean_w[u+1] + var_w[u+1]/2) for u=0..T-1
    # This is used in the zeta term contribution.
    temp_vect_topic_w = np.empty(T, dtype=current_mean_word.dtype)
    for u_zeta_idx in range(T):
        temp_vect_topic_w[u_zeta_idx] = np.exp(
            current_mean_word[u_zeta_idx + 1]
            + current_variance_word[u_zeta_idx + 1] / 2.0
        )
    ...
    for t_obs_idx in range(T):  # t_obs_idx for d/d(obs_t), from 0 to T-1
        ...
        for u_time_idx in range(T):  # u_time_idx from 0 to T-1
            ...
            if zeta_u > 0.0:
                # (zeta_u - exp_term_w_u) / (zeta_u**2) is d(1/zeta_u_approx_wrt_mw)/dm_w
                # Simplified derivative of (exp_term_w / zeta_u) w.r.t mean_w[u+1]
                # d/dm (exp(m+v/2)/Z) = (exp(m+v/2)/Z) * (1 - exp(m+v/2)/Z) approx.
        ...
        for u_time_idx in range(T):
            ...
            if chain_variance > 0.0:
                ...
                # Derivative of -(-v_w[u+1]/cv - log(cv)) w.r.t obs_t is 0
                # as v_w is fixed during obs optimization.

        # Part 3: Prior term for m_w[0]
        # Derivative of -m_w[0]^2 / (2 * INIT_MULT_NB * chain_variance)
        if chain_variance > 0.0 and INIT_MULT_NB > 0.0:
            deriv_val_for_obs_t -= (
                current_mean_word[0]
                / (INIT_MULT_NB * chain_variance)
                * mean_deriv_for_obs_t[0]
            )

        deriv_output[t_obs_idx] = deriv_val_for_obs_t


@njit
def numba_f_obs(
    x_obs_w: np.ndarray,  # Arg 0 (from x0 passed by fmin_cg)
    # Start of args tuple (12 elements)
    word_counts_w: np.ndarray,  # Arg 1 (from args_for_optimizer[0])
    totals_time: np.ndarray,  # Arg 2 (from args_for_optimizer[1])
    variance_word_fixed: np.ndarray,  # Arg 3 (from args_for_optimizer[2])
    fwd_variance_word_fixed: np.ndarray,  # Arg 4 (from args_for_optimizer[3])
    chain_variance: float,  # Arg 5 (from args_for_optimizer[4])
    obs_variance_scalar: float,  # Arg 6 (from args_for_optimizer[5])
    num_time_slices: int,  # Arg 7 (from args_for_optimizer[6])
    zeta_topic_fixed: np.ndarray,  # Arg 8 (from args_for_optimizer[7])
    mean_word_buffer: np.ndarray,  # Arg 9 (from args_for_optimizer[8])
    fwd_mean_word_buffer: np.ndarray,  # Arg 10 (from args_for_optimizer[9])
    mean_deriv_mtx_w_unused: np.ndarray,  # Arg 11 (from args_for_optimizer[10]) - Ignored by f_obs
    grad_output_buffer_unused: np.ndarray,  # Arg 12 (from args_for_optimizer[11]) - Ignored by f_obs
) -> float:
    """
    Numba-jitted version of the objective function f_obs.
    Calculates the negative of the objective value.
    """
    T = num_time_slices
    current_objective_val = 0.0

    # Compute mean_w based on current x_obs_w
    numba_compute_post_mean(
        x_obs_w,
        fwd_variance_word_fixed,
        mean_word_buffer,
        fwd_mean_word_buffer,  # Outputs
        chain_variance,
        obs_variance_scalar,
        num_time_slices,
    )
    # mean_word_buffer now holds the mean consistent with x_obs_w

    # Objective terms (signs for maximization of the true objective)
    for t_idx in range(T):  # t_idx from 0 to T-1
        m_wt_plus_1 = mean_word_buffer[t_idx + 1]
        m_wt = mean_word_buffer[t_idx]
    ...
    if chain_variance > 0.0 and INIT_MULT_NB > 0.0:
        current_objective_val -= np.power(mean_word_buffer[0], 2) / (
            2 * INIT_MULT_NB * chain_variance
        )

    return -current_objective_val  # fmin_cg minimizes


@njit
def numba_df_obs(
    x_obs_w: np.ndarray,  # Arg 0 (from x0)
    # Start of args tuple
    word_counts_w: np.ndarray,  # Arg 1 (tuple[0])
    totals_time: np.ndarray,  # Arg 2 (tuple[1])
    variance_word_fixed: np.ndarray,  # Arg 3 (tuple[2])
    fwd_variance_word_fixed: np.ndarray,  # Arg 4 (tuple[3])
    chain_variance: float,  # Arg 5 (tuple[4])
    obs_variance_scalar: float,  # Arg 6 (tuple[5])
    num_time_slices: int,  # Arg 7 (tuple[6])
    zeta_topic_fixed: np.ndarray,  # Arg 8 (tuple[7])
    mean_word_buffer: np.ndarray,  # Arg 9 (tuple[8])
    fwd_mean_word_buffer: np.ndarray,  # Arg 10 (tuple[9])
    mean_deriv_mtx_w: np.ndarray,  # Arg 11 (tuple[10]) - Used
    grad_output_buffer: np.ndarray,  # Arg 12 (tuple[11]) - Used as output buffer for gradient
) -> np.ndarray:
    """
    Numba-jitted version of the gradient function df_obs.
    Calculates the negative of the true objective's gradient.
    """
    # Compute mean_w based on current x_obs_w, as compute_obs_deriv needs it
    numba_compute_post_mean(
        x_obs_w,
        fwd_variance_word_fixed,
        mean_word_buffer,
    ...
    numba_compute_obs_deriv(
        ...
        totals_time,
        mean_deriv_mtx_w,
        grad_output_buffer,  # Output
        chain_variance,
        num_time_slices,
    )

    # fmin_cg minimizes, so pass negative of true objective's gradient
    return np.negative(grad_output_buffer)
...
class LdaSeqModel(utils.SaveLoad):
    ...
    def fit_lda_seq(
        ...
        with tqdm.tqdm(total=em_max_iter, desc="EM Iterations") as pbar:
            while iter_ < em_min_iter or (
                ...
                if convergence < LDASQE_EM_THRESHOLD:
                    ...
                    logger.info(
                        "Starting final iterations, max iter is %i",
                        lda_inference_max_iter,
                    )
                    convergence = 1.0
...
class sslm(utils.SaveLoad):
    ...
    def fit_sslm(self, sstats):
        ...
        with tqdm.tqdm(total=sslm_max_iter, desc="SSLM iterations") as pbar:
    ...
    def update_obs(
        self, sstats: np.ndarray, totals: np.ndarray
    ) -> Tuple[np.ndarray, np.ndarray]:
        """
        Optimize the bound with respect to the observed variables (self.obs),
        using Numba-jitted functions for fmin_cg callbacks.
        """
        if self.vocab_len is None or self.num_time_slices is None:
            # Or raise an error if these being None is not permissible at this stage
            logging.error(
                "sslm not properly initialized: vocab_len or num_time_slices is None"
            )
            return self.obs, self.zeta  # Return current state if not initialized
        ...
        # Pre-allocate buffers for Numba functions (reused for each word)
        # Ensure dtype is consistent with your data (e.g., np.float64)
        buffer_dtype = self.obs.dtype if hasattr(self.obs, "dtype") else np.float64

        mean_word_buffer = np.zeros(T + 1, dtype=buffer_dtype)
        fwd_mean_word_buffer = np.zeros(T + 1, dtype=buffer_dtype)
        grad_output_buffer = np.zeros(T, dtype=buffer_dtype)
        temp_mean_deriv_row_buffer = np.zeros(T + 1, dtype=buffer_dtype)
        mean_deriv_mtx_w = np.zeros((T, T + 1), dtype=buffer_dtype)

        norm_cutoff_obs_cache: Optional[np.ndarray] = None

        for w_idx in W:  # Iterate over words
            word_counts_w = sstats[w_idx, :]  # Shape (T,)
            ...
            if run_optimization:
                ...
                if counts_norm < OBS_NORM_CUTOFF:
                    # If norm is low (and no cache hit), effectively optimize with zero counts
                    word_counts_w_for_opt = np.zeros_like(word_counts_w)

                # Fixed data for the current word (passed to Numba functions)
                # These are views or copies, ensure they are C-contiguous if Numba requires
                variance_word_fixed = np.ascontiguousarray(self.variance[w_idx, :])
                fwd_variance_word_fixed = np.ascontiguousarray(
                    self.fwd_variance[w_idx, :]
                )
                initial_obs_w = self.obs[w_idx, :]

                # Precompute mean_deriv_mtx_w for the current word w_idx
                for t_obs_idx_mdm in range(T):
                    # Assuming numba_compute_mean_deriv is defined and imported
                    numba_compute_mean_deriv(
                        fwd_variance_word_fixed,
                        temp_mean_deriv_row_buffer,  # Output buffer
                        self.chain_variance,
                        self.obs_variance,  # Scalar obs_variance of the sslm object
                        T,  # num_time_slices
                        t_obs_idx_mdm,  # Index of obs variable for derivative
                    )
                    mean_deriv_mtx_w[t_obs_idx_mdm, :] = temp_mean_deriv_row_buffer

                # Assemble all arguments for numba_f_obs and numba_df_obs
                # This tuple must match the expected *args by the Numba functions
                args_for_optimizer = (
                    word_counts_w_for_opt,
                    totals,  # totals_time in Numba func
                    variance_word_fixed,
                    fwd_variance_word_fixed,
                    self.chain_variance,
                    self.obs_variance,  # obs_variance_scalar in Numba func
                    T,  # num_time_slices
                    np.ascontiguousarray(self.zeta),  # zeta_topic_fixed in Numba func
                    # Buffers (must match order in Numba function signatures)
                    mean_word_buffer,
                    fwd_mean_word_buffer,
                    mean_deriv_mtx_w,  # For numba_df_obs
                    grad_output_buffer,  # For numba_df_obs
                )

                try:
                    optimized_obs_w_result = optimize.fmin_cg(
                        f=numba_f_obs,  # Numba-jitted objective function
                        fprime=numba_df_obs,  # Numba-jitted gradient function
                        x0=initial_obs_w,
                        args=args_for_optimizer,
                        gtol=TOL_GTOL,
                        disp=0,  # Set to 1 for convergence messages from fmin_cg
                    )
                    current_obs_w_optimized = optimized_obs_w_result
                except (
                    Exception
                ) as e:  # Catch potential errors from fmin_cg or Numba calls
                    logging.error(f"Optimization failed for word {w_idx}: {e}")
                    current_obs_w_optimized = (
                        initial_obs_w  # Fallback to initial if opt fails
                    )

                if counts_norm < OBS_NORM_CUTOFF:
                    norm_cutoff_obs_cache = np.copy(current_obs_w_optimized)

            self.obs[w_idx, :] = current_obs_w_optimized

        # After all self.obs are updated, self.mean for all words needs to be recomputed
        # as self.mean depends on self.obs. This is crucial before updating self.zeta.
        for w_idx_update_mean in range(W):
            # Assuming numba_compute_post_mean is defined and imported
            # It updates self.mean[w_idx_update_mean, :] and self.fwd_mean[w_idx_update_mean, :]
            numba_compute_post_mean(
                np.ascontiguousarray(self.obs[w_idx_update_mean, :]),
                np.ascontiguousarray(self.fwd_variance[w_idx_update_mean, :]),
                self.mean[w_idx_update_mean, :],  # Output view for mean
                self.fwd_mean[w_idx_update_mean, :],  # Output view for fwd_mean
                self.chain_variance,
                self.obs_variance,  # Scalar
                T,  # num_time_slices
            )

        self.zeta = (
            self.update_zeta()
        )  # update_zeta should use the newly computed self.mean
