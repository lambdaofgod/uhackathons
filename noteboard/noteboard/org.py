...
class OrgElement(BaseModel):
    ...
    def get_org_node_elements(cls, file_name, file_node, full_root_contents=False):
        root_element = cls.get_root_node_element(file_node, file_name)
        children_elements = [
    ...
    @classmethod
    def get_root_node_element(cls, file_node, file_name=None, full_root_contents=False):
...
class Org:
    ...
    @classmethod
    def load(cls, org_path) -> List[OrgElement]:
        file_name = Path(org_path).stem
        return cls.element_cls.get_org_node_elements(file_name, orgparse.load(org_path))

    @classmethod
    def loads(cls, org_contents, file_name=None) -> List[OrgElement]:
        return cls.element_cls.get_org_node_elements(file_name, orgparse.loads(org_contents))

    @classmethod
    def load_dir_generator(cls, dir) -> Sequence[OrgElement]:
        dir_p = Path(dir) / "**" / "*.org"
