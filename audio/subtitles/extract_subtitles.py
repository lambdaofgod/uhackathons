...
def extract_subtitles(input_file, output_file=None, language="en", model_name="medium", log_level="INFO", use_gpu=None):
    """
    Extract subtitles from a video file using ffmpeg and whisper.
    
    Args:
        input_file: Path to the input video file (.avi, .mp4, etc.)
        output_file: Path to the output subtitle file (.srt)
    ...
    logger.info(f"Starting subtitle extraction for '{input_file}'")
    ...
    try:
        ...
        create_srt_file(result["segments"], output_file, logger)
    ...
    logger.info(f"Subtitle extraction completed successfully in {time_str}")
